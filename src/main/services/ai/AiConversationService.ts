import type {
  AiSessionDetail, AiSessionSummary, AiTurn, CreateAiSessionRequest, GenerateAiTurnRequest,
...
export class AiConversationService {
  ...
  async generateTurn(rawRequest: GenerateAiTurnRequest): Promise<AiTurn> {
    ...
    try {
      const attachments = request.images ? await Promise.all(request.images.map((image) => this.attachmentService.saveImage({ workspaceId: session.workspaceId, sessionId: session.id, turnId: turn.id, image }))) : [];
      ...
      this.logger?.info("ai.turn.completed", { sessionId: session.id, turnId: turn.id });
      return this.repository.getTurn(turn.id) ?? { ...turn, status: "ready", mermaid, attachments };
    } catch (error) {
      const message = error instanceof Error ? error.message : "AI 图表生成失败";
      this.repository.failTurn(turn.id, message);
      this.logger?.warn("ai.turn.failed", { sessionId: session.id, turnId: turn.id, message });
      throw error;
    }
  }
