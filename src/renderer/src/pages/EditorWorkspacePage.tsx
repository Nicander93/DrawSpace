...
interface EditorLocationState { initialContent?: DocumentContent }
...
export function EditorWorkspacePage({ visible = true }: { visible?: boolean }) {
  ...
  const openTab = useCallback(async (document: CanvasDocument, content?: DocumentContent) => {
    openDocument({
    ...
    navigate(`/editor/${document.id}`, { replace: true, state: content ? { initialContent: content } : undefined });
    setPickerOpen(false);
  ...
  useEffect(() => {
    ...
    if (document) {
      openDocument({ documentId: document.id, name: document.name, relativePath: document.relativePath, isFavorite: document.isFavorite });
      activateDocument(document.id);
      return;
    }
    void window.desktopApi.documents.open(documentId).then((content) => {
      void openTab(content.document, state?.initialContent ?? content);
    });
  ...
  useEffect(() => {
    ...
    const handleKeyboard = (event: KeyboardEvent): void => {
      ...
      if (modifier && event.key.toLowerCase() === "n") {
        event.preventDefault();
        void window.desktopApi.documents.create().then((content) => openTab(content.document, content));
        return;
      }
  ...
  const registerDiscard = useCallback((id: string, discard: () => void) => {
  ...
  const discardAndExit = useCallback(() => {
    ...
    void Promise.all(targetTabs.map((tab) => window.desktopApi.recovery.discard(tab.documentId)))
      .then(() => {
        targetTabs.forEach((tab) => discardHandlersRef.current.get(tab.documentId)?.());
      })
  ...
  return (
    <div className="editor-workspace">
      <div className="editor-workspace__bar">
        <button className="editor-workspace__back" type="button" onClick={returnToWorkspace}><ArrowLeft size={16} /> 工作区</button>
        <span>已打开 {tabs.length} 个画布</span>
        {activeTab && (
          <div className={`editor-workspace__save-status editor-workspace__save-status--${activeTab.saveStatus}`} title={activeTab.saveError ?? undefined}>
            ...
            {activeTab.saveStatus === "error" && <FileWarning size={13} />}
            {activeTab.saveStatus === "conflict" && <FileWarning size={13} />}
            ...
            {activeTab.saveStatus === "error" && <button type="button" aria-label="重试保存" onClick={() => void saveHandlersRef.current.get(activeTab.documentId)?.()}><RefreshCw size={13} /></button>}
          </div>
        )}
      </div>
      <EditorTabs tabs={tabs} activeDocumentId={activeDocumentId} onActivate={activate} onClose={(id) => { void closeTabsWithGuard([id]); }} onAdd={() => setPickerOpen(true)} onReorder={reorderTabs} onContextMenu={handleTabContextMenu} />
      <div className="editor-document-host">
        {tabs.map((tab) => (
          <div className={`editor-document-pane ${tab.documentId === activeDocumentId ? "is-active" : ""}`} aria-hidden={tab.documentId !== activeDocumentId} key={tab.documentId}>
            <EditorPage documentId={tab.documentId} embedded active={tab.documentId === activeDocumentId} registerSave={registerSave} registerClose={registerClose} registerDiscard={registerDiscard} onClose={() => closeDocument(tab.documentId)} />
          </div>
        ))}
      ...
      {pickerOpen && (
        <DocumentPicker documents={documents} tabs={tabs} onOpen={(document) => void openTab(document)} onClose={() => setPickerOpen(false)} />
      )}
      {closeDialogOpen && (
        <UnsavedDocumentsDialog intent={closeIntent} tabs={(closeIntent === "tabs" ? tabs.filter((tab) => pendingTabCloseIds?.includes(tab.documentId)) : tabs).filter((tab) => tab.saveStatus !== "saved")} error={closeError} busy={closeBusy} onSaveAll={saveAllAndExit} onDiscard={discardAndExit} onCancel={cancelExit} />
      )}
      {tabMenu && (
        <div className="context-menu editor-tab-context-menu" style={{ left: tabMenu.x, top: tabMenu.y }} onMouseLeave={() => setTabMenu(null)}>
          <button type="button" onClick={() => { activate(tabMenu.tab.documentId); window.dispatchEvent(new CustomEvent("canvasdesk:request-rename", { detail: tabMenu.tab.documentId })); setTabMenu(null); }}>Rename</button>
